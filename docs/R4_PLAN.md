...
| **R4.1** | UX foundation: project → session tree navigation (project switcher, session list per project, create-session inline form), theme system extended to the external bar (custom-color override UI; tokens/presets/dark default were shipped by M1.9 step 1), app shell redesigned, **scaffold-first for R4.1 surfaces only** (Memory/Agents/Settings stay in R4.4); **backend WS events** (`project.created` / `project.deleted` / `session.created` / `session.deleted` / `active_session.changed` — the `projects.py` router does not publish today) | ~1 | `docs/R4_1_PLAN.md` (rewritten 2026-09-05 — planning review collapse + cite, ruling #3 WS events, ruling scaffold-scope, ruling inline-form) | **Done 2026-09-06** (4 implementation commits + 1 e2e/docs commit; 458/458 pytest, 60 vitest, 13/13 run.py --check, npm run build green). Amended 2026-09-06: stack upgrade (React 19 + Tailwind v4) inserted as step 1c; R4.2/R4.3 adopt assistant-ui runtime + agent-elements-derived cards. |
| **R4.2** | Chat surface to the quality bar: streaming (chat.delta), markdown/code rendering, serial-turn indicator, session lifecycle in-thread (closes M1.9 funnel leak #1) | ~0.75 | `docs/R4_2_PLAN.md` | Direction locked: assistant-ui custom runtime adapter + agent-elements-derived tool cards; React 19 / Tailwind v4 required (R4.1 step 1c). Planned in detail: `docs/R4_2_PLAN.md` (adapter spike gate: assistant-ui opencode adapter vs useExternalStoreRuntime custom adapter; backend stays source of truth). **Amendment 2026-09-07**: steps 0 + 1 + 2a shipped; user rejected the `/chat` surface as "unpolished and frankly bad." Visual polish is the bulk of step 2, not a step-3 finish - design spec in `docs/R4_2_VISUAL_POLISH.md`; planner to insert a polish step (2-pre, ~0.6) before 2b/2c/3. IN EXECUTION: steps 0/1/2a done (adapter rejected w/ evidence, custom useExternalStoreRuntime adapter, markdown); user rejected /chat visuals -> hand-back; step 2-pre (polish, LibreChat bar) = own round with user visual gate (rulings 2026-09-07: stop wired, edit deferred to R4.3, lab replaced).
| **R4.3** | Output funnel to the quality bar: children live tree (already partly in wave 1 — rebuilt to the bar), escalation lane (`ask_human` answer inline), promote inline, delegation detail view (composed prompt, tool timeline, tokens/cost, output — from M1.9's parts-model traces) — closes funnel leaks #2–#4 | ~0.75 | `docs/R4_3_PLAN.md` | Direction locked: same stack; tool timeline + escalations as generative-UI components in the thread.
| **R4.4** | Memory pane, Agents workbench, Settings + model catalog (the drafted wave-2 spec, re-cut post-R4.1) | ~1.5 | rename `docs/R4_1_PLAN.md` → `docs/R4_4_PLAN.md` | + **Specialist gallery** (LobeChat-Market-inspired: browse/install presets via Omnigent-spec YAML)
