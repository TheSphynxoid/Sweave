...
| Provider auth for the FULL catalog (no provider left behind) | Partial (step-1 constraint live for openrouter/zai/ollama/gmicloud/nvidia; 2026-09-14 Go slice: `opencode-go` mapped — public `/zen/go/v1` endpoints, pasted-key auth via env/bootstrap, chat/completions live; the google flavor fails loud `bad_request` naming the pending transport; loop-path `resolved.ok` gate closed. 2026-09-19 night: `zai-coding-plan` mapped (separate /coding/ endpoint per models.dev, chat flavor, no sibling-key fallback) and `thinkingmachines` mapped (first-party Anthropic-native tinker endpoint per models.dev, messages flavor + x-api-key auth mode, per-provider messages default); both keyed via env/Sweave-store/opencode-auth-store bootstrap (entries exist on real machines). Still unmapped: `gmi` (no models.dev entry, no known endpoint, no credential anywhere — needs endpoint proof + a key before mapping). Validated-client headers (Go docs "Where can I use it"): `User-Agent: sweave-engine/0.1.0` + stable `x-opencode-session` (durable eng_* id) on every provider call, single-shot and loop iterations, pinned hermetically. Still open: google transport (gemini rows fail loud naming it), Zen live proof (chat mapped + hermetic green 2026-09-14; live: Bearer accepted/key valid, but deepseek-free → 400 unavailable and muse-free → 500 — server-side, $0 spent; needs a servable model or one approved paid micro-turn), thinkingmachines/gmi endpoint probes, copilot deferred, engine OAuth long-term). Responses transport DONE hermetic 2026-09-14 (`responses.js`: text deltas + function_call items + usage mapping, single-shot + loop, identical trace vocabulary; live proof needs a responses-model micro-turn). Messages transport DONE hermetic 2026-09-19 (`messages.js`: text/tool_use/tool_result blocks + role-alternation merge + usage mapping, single-shot + loop, identical trace vocabulary; max_tokens 32000 default with halve-till-4096 strip-retry; thinking blocks dropped (no replayable signatures) and +variant effort not sent (budgets need live proof) — both documented in-file; live proof needs a messages-model micro-turn). 2026-09-19 night: +variant effort now SENT, ported from opencode's own variants() (sst/opencode transform.ts, same gateway family): adaptive-listed values ride `{thinking:{type:adaptive}[+summarized], effort}` top-level, older high/max compute budgetTokens from our max_tokens (min(16000, maxTokens/2-1) / min(31999, maxTokens-1), always < max_tokens), minimax toggle/none mapped, unknowns ride adaptive+verbatim with the thinking strip-retry as backstop; refusals classify via the variant_refused taxonomy event. 2026-09-14 credential-ownership (user ruling): `~/.sweave/credentials.json` (0600) is canonical — adopt-once from opencode store (ledgered), drift→pending-import prompts, reverse-sync ours→theirs (backup kept), API-key types only, OAuth detect-only; local/keyless + custom endpoints explicitly out of scope; sidecar reads the Sweave tier (env → sweave → opencode-legacy); `GET /api/providers` = universe × availability; keychain UI rides the next slice |
...
| Per-specialist selection + opencode fallback | Step 4 done (default flipped, per-task override, fallback-only-before-work). AMENDED 2026-09-14 (user ruling): no automatic cross-harness fallback — fail loud across harnesses, fail over within. REMOVAL EXECUTED 2026-09-14 (user ruling: clarity over obscurity): `run()` no longer retries engine failures on opencode — pre-work engine death fails loud as `[chat error: engine_failed_before_work: ...]` (single wrap); doctrine: selection sticks, reliability is per-harness (health gate + in-harness provider failover). Live note: `Sweave-20260914-022215-630a55` proves native engine live (chat-flavor opencode-go turn, no fallback); the responses-flavor live proof is still open (needs one responses-model micro-turn) |
