...
| **Engine sidecar (chat + tools)** | ✅ | Custom-engine steps 1–2, 2026-09-13 — zero-dependency Node `sweave-engine/src/` (`serve.js` + `loop.js` + `tools.js` + `sweave.js` + `sessions.js` + `providers.js`): true token streaming, 8-tool executor (read/edit/write/bash/glob/grep/todo/git) with blind permission-map enforcement (allow/ask/deny, last-match-wins, external_directory), agentic loop with per-tool budgets + doom-loop guard + structural role gate, durable sessions, full-catalog auth (named `auth_missing`, never cryptic). Terminal provider failures on BOTH single-shot flavors end the turn loudly in place (failed record + error SSE + res.end — the responses branch used to re-throw past committed SSE headers, hanging the client until the turn timeout; fixed 2026-09-14 after the zen suite wedged). Hygiene 2026-09-20: journal is per-session shards (`sessions/<id>.json`, lazy index, debounced appends, turn-end flush, legacy import + empty-GC, ensure-after-validate); read lines N:-numbered; glob recurses + `literal`; git teaches + status-default; charters render a per-turn `{{tools}}` truth line. Opt-in only — opencode stays the default until step-4 parity flip |
...
| **Per-part versioning** | ✅ | 2026-09-20 (user-locked: per-part + contracts, automate mechanics, endpoint now) — backend/engine/web carry independent semver (`pyproject.toml`, `sweave-engine/package.json`, `sweave-web/package.json`); compatibility gates on the CONTRACTS (engine protocol, Delegation schema, frozen trace vocab), never the part numbers. `scripts/bump_versions.py` is the only writer (patch = fixes, minor = milestone/format/behavior, major reserved for 1.0; tags `<part>-vX.Y.Z`); `sweave/version.py` is the single reader behind `GET /api/version` + `sweave version`; sidecar `/health` names its part version. Seeded: engine 0.2.0 (journal shards), backend 0.1.1, web 0.1.0 |
...
| **Session lazy-load (meta/jsonl split)** | ✅ | 2026-09-20 (user ruling: lazy-load before search) — session schema v1→v2: `{id}.json` carries meta only (+ `message_count`), bodies live in `{id}.messages.jsonl` (one row per line, UTF-8, `ensure_ascii=False` like the meta writer). Boot parses metas only (25 live sessions / 433 messages migrate once on load, verified); `get_session` faults one transcript (LRU `BODY_CACHE_CAP=50`, idle-only eviction); `list/active` paths never fault (active returns the Summary shape the UI already types). Writes: `append_message` is the O(1) hot path (one line + meta bump); `save_session` is meta-only; `save_messages` rewrites the log for history edits (rerun supersede flags). Bridges + titling pre-checks use `get_session_meta`. Backend minor bump 0.1.2→0.2.0 (on-disk format change per the bump rules); web/engine untouched |
